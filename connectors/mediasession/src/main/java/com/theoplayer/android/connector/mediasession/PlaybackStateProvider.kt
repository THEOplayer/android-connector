...
class PlaybackStateProvider(private val connector: MediaSessionConnector) {
    ...
    private fun registerListeners() {
        player?.apply {
            addEventListener(PlayerEventTypes.LOADSTART, onLoadStart)
            ...
            addEventListener(PlayerEventTypes.READYSTATECHANGE, onReadyStateChange)
    ...
    private fun unregisterListeners() {
        player?.apply {
            removeEventListener(PlayerEventTypes.LOADSTART, onLoadStart)
            ...
            removeEventListener(PlayerEventTypes.READYSTATECHANGE, onReadyStateChange)
    ...
    private val onLoadStart = { _: LoadStartEvent ->
    ...
    private val onReadyStateChange = { event: ReadyStateChangeEvent ->
        if (event.readyState.ordinal < ReadyState.HAVE_CURRENT_DATA.ordinal) {
            updatePlaybackState(PlaybackStateCompat.STATE_BUFFERING)
